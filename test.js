const test = () => {
  ...
  function openDb() {
    ...
    req.onupgradeneeded = function (evt) {
      ...
      let store = evt.currentTarget.result.createObjectStore(DB_STORE_NAME, { keyPath: 'id', autoIncrement: true });
      ...
      if (cursor) {
        ...
        req.onsuccess = function (evt) {
          ...
          if (value.hasOwnProperty('blob') &&
            ...
            let link = $('<a href="' + cursor.key + '">File</a>');
  ...
  function setInViewer(key) {
    ...
    getBlob(key, store, function(blob) {
      ...
      if (blob.type == 'text/html') {
        ...
        iframe.load(function() {
          let img_id = 'image-' + key;
          let img = $('<img id="' + img_id + '"/>');
  ...
  function addEventListeners() {
    ...
    $('#add-button').click(function(evt) {
      console.log("add ...");
      let title = $('#pub-title').val();
      let biblioid = $('#pub-biblioid').val();
      if (!title || !biblioid) {
        displayActionFailure("Required field(s) missing");
        return;
      }
      let year = $('#pub-year').val();
      if (year != '') {
      ...
      let file_input = $('#pub-file');
      let selected_file = file_input.get(0).files[0];
      console.log("selected_file:", selected_file);
      // Keeping a reference on how to reset the file input in the UI once we
      // have its value, but instead of doing that we rather use a "reset" type
      // input in the HTML form.
      //file_input.val(null);
      let file_url = $('#pub-file-url').val();
      if (selected_file) {
    ...
    $('#delete-button').click(function(evt) {
      console.log("delete ...");
      let biblioid = $('#pub-biblioid-to-delete').val();
    ...
    let search_button = $('#search-list-button');
