...
import { Boolean, FormItem, Icon, Label, Select, FilterTagItem, Input } from "../../../dol";
import { useEffect, useMemo } from "react";
import { isEmpty } from "../../../../globals/functions";

export const SmartFilters = (props) => {
        ...
        return (
            <div className={`col lg:row-v-center border border-dol divide-y lg:divide-x divide-dol`}>
                <div className={`col divide-y divide-dol basis-1/2`}>
                    <Select value={selectedAttributeName} onChange={(newState) => { set("selectedAttributeName", newState); set("filterMode", "inclusive"); }} options={Object.entries(attributes).map(([key, attribute]) => ({ label: attribute.label, value: key }))} />
                    <div className={`col divide-y divide-dol`}>
                        <div className={`row-v-center divide-x divide-dol`}>
                            ...
                            <button onClick={() => set("filterMode", "exclusive")} className={`row-full-center flex-grow duration-100 gap-2 p-2 ${filterMode === "exclusive" ? "bg-error-20 text-error" : "bg-dol text-soft-dol button-dol"}`}>
                                <Icon library={`fa6`} icon={`FaBan`} />
                                <span>Filtre exclusif</span>
                            </button>
                        </div>
                        <div className={`col gap-6 p-6`}>

                            {"strict" in selectedFilter && <Boolean label={"Filtre Strict"} value={selectedFilter.strict} onChange={value => set(`${selectedSkelFilterLink}.strict`, value)} />}
                            <FormItem value={selectedFilter.value} onChange={value => set(`${selectedSkelFilterLink}.value`, value)} { ...attributes[selectedAttributeName]} label={`Valeur`} />
                            {"interval" in selectedFilter && 
                            <Label label={`Intervalle`}>
                                <div className={`row-v-center gap-4`}>
                                    <Input placeholder={`Minimum ...`} value={selectedFilter.interval.min} onChange={value => set(`${selectedSkelFilterLink}.interval.min`, value)} />
                                    <Input placeholder={`Maximum ...`} value={selectedFilter.interval.max} onChange={value => set(`${selectedSkelFilterLink}.interval.max`, value)} />
                                </div>
                            </Label>
                            }
                                        ...
                                        <div key={"inclusiveFilter" + FI} className={`row-between-center gap-2 py-2`}>
                                            {(!inclusive.value && inclusive.interval) &&
                                                <>
                                                    {inclusive.interval.min && 
                                                        <>
                                                            <span><FilterTagItem type={type} value={inclusive.interval.min} /></span>
                                                    ...
                                                    {inclusive.interval.max && 
                                                        <>
                                                            <span className={`text-3xl text-success`}>≤</span>
                                                            <span><FilterTagItem type={type} value={inclusive.interval.max} /></span>
                                                        </>
                                                    }
                                                </>
                                            }
                                            {!isEmpty(inclusive.value) &&
                                                <>
                                                    ...
                                                    <span><FilterTagItem type={type} value={inclusive.value} /></span>
                                                </>
                                            }
                                        </div>
                                    }
                                        ...
                                        <div key={"exclusiveFilter" + FI} className={`row-between-center gap-2 py-2`}>
                                            {(!exclusive.value && exclusive.interval) &&
                                                <>
                                                    {exclusive.interval.min && 
                                                        <>
                                                            <span><FilterTagItem type={type} value={exclusive.interval.min} /></span>
                                                    ...
                                                    {exclusive.interval.max && 
                                                        <>
                                                            <span className={`text-3xl text-error`}>≤</span>
                                                            <span><FilterTagItem type={type} value={exclusive.interval.max} /></span>
                                                        </>
                                                    }
                                                </>
                                            }
                                            {!isEmpty(exclusive.value) &&
                                                <>
                                                    ...
                                                    <span><FilterTagItem type={type} value={exclusive.value} /></span>
                                                </>
                                            }
                                        </div>
                                    }
